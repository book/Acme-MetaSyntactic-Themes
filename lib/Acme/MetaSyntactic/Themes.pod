...
With the version 1.000 of Acme::MetaSyntactic, I have split the core
modules (that provide the list behaviour) from the actual themes, making
the distribution containing the very stable L<Acme::MetaSyntactic> modules
driving its behaviour a lot thinner.

When Acme-MetaSyntactic 0.99 was published, the distribution contained
99 themes. The full list of theme and the date of their inclusion in
the distribution is documented below:
...
These themes are: C<abba>, C<zodiac>.
...
Future releases will contain more themes proposed in the pre-1.000 era,
as well as updates to the themes already in the distribution.

New themes don't need to be sent to me for inclusion in this
distribution. They can (and probably should) be distributed in their
own distribution (see for example L<Acme::MetaSyntactic::buzzwords>,
L<Acme::MetaSyntatic::daleks>, L<Acme::MetaSyntatic::nethack>, and
L<Acme::MetaSyntatic::vim>).
To make your own Acme::MetaSyntactic theme distribution, see the F<eg/>
directory shipped with the C<Acme-MetaSyntactic> distribution.

L<Bundle::MetaSyntactic> is a bundle meant to collect all available
L<Acme::MetaSyntactic> themes. Please contact the bundle maintainer if
